...
export const tools = {
	getProjects: tool({
		description:
			"Get information about Daniel's projects. Can filter by technology, awards, or keywords.",
		inputSchema: z.object({
			technology: z
				.string()
				.optional()
				.describe('Filter by specific technology (e.g., TypeScript, Python, AI)'),
			hasAward: z.boolean().optional().describe('Filter for award-winning projects only'),
			keyword: z
				.string()
				.optional()
				.describe('Search for projects containing this keyword in title or description'),
				...
				positions: [
					{
						...
						description:
							'Research and development work in medical imaging. Working with Python and MRI data.'
					}
				]
			};
		}
